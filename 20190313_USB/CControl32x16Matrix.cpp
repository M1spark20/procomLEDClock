...
bool CControl32x16Matrix::TransferToMatrix(){
	...
	for(unsigned int port=0; port<4; ++port){
		for(unsigned int panel=0; panel*16<maxSize; ++panel){
			digitalWrite(Pin_OE, 0);
			
			for(unsigned int section=0; section<8; ++section){
				const unsigned int sectionBegin[][2] = {
					{ (section/2)*8, (section%2)*4+port+panel*16},
					{ (section/2)*8, (section%2)*4+port+8+panel*16},
				}; // [0, 1][x, y]
				
				unsigned int transferData[][2] = {
					{ 0, 0 }, { 0, 0 }, { 0, 0 }
				};// [0,1][R,G,B]
				...
				for(unsigned int dots=0; dots<8; ++dots){
					...
					digitalWrite(Pin_R2, Decode(transferData[0][0], showPosX[0], false));
					digitalWrite(Pin_G2, Decode(transferData[1][0], showPosX[0], false));
					digitalWrite(Pin_B2, Decode(transferData[2][0], showPosX[0], false));
					digitalWrite(Pin_R3, Decode(transferData[0][1], showPosX[1], false));
					digitalWrite(Pin_G3, Decode(transferData[1][1], showPosX[1], false));
					digitalWrite(Pin_B3, Decode(transferData[2][1], showPosX[1], false));
