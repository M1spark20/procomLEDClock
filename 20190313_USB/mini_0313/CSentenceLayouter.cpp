...
CSentenceLayouter::CSentenceLayouter(CControl64x32Matrix& matrix) : m_matrixManager(matrix){
	m_showData[0].brightness.resize(32);
	m_showData[0].colorData.resize(32);
	m_terminateFlag = false;
	for(auto i=0; i<m_showData[0].brightness.size(); ++i){
		m_showData[0].brightness[i].resize(64*2);
		m_showData[0].colorData[i].resize(64*2);
	}
...
void CSentenceLayouter::flushProcess(){
	...
	while(!m_terminateFlag){
		...
		CControl64x32Matrix::TExpressData dd[3];
		for(int i=0; i<3; ++i)	dd[i].resize(64, 0);
		
		for(unsigned int y=0; y<m_pUsingData->brightness.size(); ++y)
		for(unsigned int x=0; x<m_pUsingData->brightness[y].size(); ++x)
		for(int i=0; i<3; ++i){
			const unsigned int pos[] = { x%64u, y+(x/64u)*32u };
