...
*/
void CBMPManager::ReadBmp(char *filename, img *imgp) {
	int i, j;
	int Real_width;
	FILE *Bmp_Fp;
...
*/
void CBMPManager::WriteBmp(char *filename, img *tp) {
...
/*
   ...
   動作  : 引数として与えられるファイル名を持つ bmp 形式の画像ファイル
		   の属性を画面に出力する.
*/
void CBMPManager::PrintBmpInfo(char *filename) {
...
/*
   ...
   動作  : 2次元配列 tp->data の画像を水平方向の鏡映をとったものを
		   2次元配列 sp->data に格納する.
*/
void CBMPManager::HMirror(img* sp, img *tp) {
	int i, j;
	long k;
	for (i = 0; i < tp->height; i++)
		for (j = 0; j < tp->width; j++)
...
/*
   ...
   動作  : 2次元配列 tp->data の画像を垂直方向の鏡映をとったものを
		   2次元配列 sp->data に格納する.
*/
void CBMPManager::VMirror(img *sp, img *tp) {
	int i, j;
	long k;
	for (i = 0; i < tp->height; i++)
		for (j = 0; j < tp->width; j++)
...
*/
void CBMPManager::Rotate90(int a, img *sp, img *tp) {
	int i, j;
	if ((a % 4) == 0) {
		for (i = 0; i < tp->height; i++)
			for (j = 0; j < tp->width; j++)
...
/*
   ...
   動作  : 2次元配列 tp->data の画像の幅と高さを 1/a 倍した画像を
		   2次元配列 sp->data に格納する.
*/
void CBMPManager::Shrink(int a, img *sp, img *tp) {
...
/*
   ...
   動作  : 2次元配列 tp->data の画像にモザイクをかけた画像を
		   ...
		   a×a である.
*/
void CBMPManager::Mosaic(int a, img *sp, img *tp) {
...
/*
   ...
   動作  : 2次元配列 tp->data の画像をグレイスケール変換して,
		   2次元配列 sp->data に格納する.
*/
void CBMPManager::Gray(img *sp, img *tp) {
	int i, j;
	unsigned char tmp;
...
*/
void CBMPManager::Diminish(img *sp, img *tp, unsigned char x) {
	int i, j;
	unsigned char y;
